...
// 마지막 동적 import 실패 메시지 (헬스 체크/디버그용)
    ...
    while (attempt <= MAX_RETRIES) {
      try {
        const out = await withTimeout(
          // 템플릿/일반 호출 모두 함수 apply 로 통일 처리
          // lazyClient 는 실제 neon 클라이언트를 감싼 함수
          // @ts-ignore
 ...
 * dbHealth
 *
 * - Cloudflare 미들웨어/헬스 체크에서 사용.
 * - **여기에서 발생하던 파라미터 개수 오류를 수정**:
 *   예전 코드: `await sql([DEFAULT_HEALTH_SQL]);`
 *   → 내부적으로 "파라미터 1개"로 인식되어
 *     `bind message supplies 1 parameters, but prepared statement "" requires 0`
 *     에러가 발생할 수 있었음.
 *
 * - 수정 코드: 일반 호출 방식 `sql(DEFAULT_HEALTH_SQL)` 사용
 *   → 단순 텍스트 쿼리, 파라미터 0개로 안전하게 실행.
 */
export async function dbHealth(
  ...
  try {
    ...
    // ✅ 핵심 수정:
    // 파라미터 없이 순수 텍스트 쿼리로 health check 수행.
    // (Neon tagged template가 아니라 "일반 함수 호출" 방식)
